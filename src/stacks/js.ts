...
export async function jsConfig(config: Config) {
    ...
    const questions = [
        {
            ...
            default: config.get(ConfigField.trans_path, 'locales'),
    ...
    const jsResponses = await inquirer.prompt(questions);
    config.set(ConfigField.trans_path, jsResponses.trans_path);
    config.set(ConfigField.trans_filename, jsResponses.trans_filename);
    if (!config.has(ConfigField.strings)) config.set(ConfigField.strings, []);
}

export default class JSUpdate {
    ...
    private async sendStep(config: Config, stringsToSend: Array<string>, force: boolean) {
        ...
        await axios.post(
            config.get(ConfigField.host) + '/api/v1/js/texts/',
    ...
    private async retrieveLangStep(config: Config, lang: LangResponse) {
        ...
        await axios.get(
            config.get(ConfigField.host) + `/api/v1/js/langs/${lang.code}/`, {
                auth: {
                    username: config.get(ConfigField.app_id),
                    password: config.get(ConfigField.app_key)
                },
    ...
    private async retrieveLangsStep(config: Config, reset: boolean): Promise<[LangResponse] | null> {
        ...
        await axios.get(
            config.get(ConfigField.host) + '/api/v1/js/langs/', {
                auth: {
                    username: config.get(ConfigField.app_id),
                    password: config.get(ConfigField.app_key)
                },

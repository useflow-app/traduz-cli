...
export async function flutterConfig(config: Config) {
    ...
    const flutterResponses = await inquirer.prompt(questions);
    config.set(ConfigField.trans_path, flutterResponses.trans_path);
    config.set(ConfigField.trans_filename, flutterResponses.trans_filename);
    if (!config.has(ConfigField.strings)) config.set(ConfigField.strings, []);
}

export default class FlutterUpdate {

    private parserStep(config: Config, force: boolean): Array<string> {
        const Parser = require('i18next-scanner').Parser;

        CliUx.ux.action.start('parsing files *.dart');
        const files: Array<string> = glob.sync('./**/*.dart');
    ...
    private async sendStep(config: Config, stringsToSend: Array<string>, force: boolean) {
        ...
        await axios.post(
            config.get(ConfigField.host) + '/api/v1/flutter/texts/',
    ...
    private async retrieveLangStep(config: Config, lang: LangResponse) {
        ...
        await axios.get(
            config.get(ConfigField.host) + `/api/v1/flutter/langs/${lang.code}/`, {
                auth: {
                    username: config.get(ConfigField.app_id),
                    password: config.get(ConfigField.app_key)
                },
    ...
    private async retrieveLangsStep(config: Config, reset: boolean): Promise<[LangResponse] | null> {
        ...
        await axios.get(
            config.get(ConfigField.host) + '/api/v1/flutter/langs/', {
                auth: {
                    username: config.get(ConfigField.app_id),
                    password: config.get(ConfigField.app_key)
                },
